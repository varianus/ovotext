...
begin
  ...
  FLeftGutter := ((Printer.Canvas) as printer.CanvasClass).LeftMargin;
  FTopGutter := ((Printer.Canvas) as printer.CanvasClass).TopMargin;
  FRightGutter := FPhysicalWidth - FPrintableWidth - FLeftGutter;
  FBottomGutter := FPhysicalHeight - FPrintableHeight - FTopGutter;
  FXPixPrInch := Printer.XDPI; //GetDeviceCaps(Printer.Handle, LogPixelsX);
  FYPixPrInch := Printer.yDPI;//GetDeviceCaps(Printer.Handle, LogPixelsY);  }
  FXPixPrmm := FXPixPrInch / 25.4;
  FYPixPrmm := FYPixPrInch / 25.4;
end;
