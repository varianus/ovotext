...
type
  ...
  public     //Atributos y sus propiedades de acceso
    ...
    tnDirective: integer;  //id para los comentarios
...
begin
  ...
  begin  //caracter escapado
    ...
    begin //se supone que es de tipo \A
      //secuencia normal de dos caracteres
      if convert then Result := txt[2]//hay que convertirlo
      else
        Result := copy(txt, 1, 2)//lo toma tal cual
      ;
      txt := copyEx(txt, 3);
    end;
  end
...
begin
  ...
  while (f > 0) and (txt[f - 1] = '\') do f := PosEx(ch, txt, f + 1);
  Result := f;
end;
...
begin
  ...
  if UpCase(copy(exp, 1, 7)) = 'TOKPOS=' then
  begin
  ...
  if copy(exp, 1, 2) = '\d' then exp := '[0-9]' + copyEx(exp, 3)
  else if copy(exp, 1, 2) = '\D' then exp := '[^0-9]' + copyEx(exp, 3)
  else if copy(exp, 1, 2) = '\a' then exp := '[A-Za-z]' + copyEx(exp, 3)
  else if copy(exp, 1, 2) = '\w' then exp := '[A-Za-z0-9_]' + copyEx(exp, 3)
  else if copy(exp, 1, 2) = '\W' then exp := '[^A-Za-z0-9_]' + copyEx(exp, 3)
  else if copy(exp, 1, 2) = '\s' then exp := ' ' + copyEx(exp, 3)
  else if copy(exp, 1, 2) = '\S' then exp := '[^ ]' + copyEx(exp, 3)
  else if copy(exp, 1, 2) = '\t' then exp := '\x09' + copyEx(exp, 3)
  else if copy(exp, 1, 1) = '.' then exp := '[\x01-\xFF]' + copyEx(exp, 2);
  //analiza la secuencia
  if (exp[1] = '[') and (length(exp) > 1) then
  begin    //Es lista de caracteres
    //Captura interior del intervalo.
    exp := CopyEx(exp, 2);
    listChars := '';
    tmp := ExtractCharN(exp);   //No convierte para no confundir "\]"
    while (exp <> '') and (tmp <> ']') do
    begin
      listChars += tmp;
      tmp := ExtractCharN(exp);  //No convierte para no confundir "\]"
    end;
    ...
    if exp = '' then Result := tregChars//Lista de tipo "[ ... ]"
    else if exp[1] = '*' then
    begin  //Lista de tipo "[ ... ]* ... "
    ...
    begin  //Lista de tipo "[ ... ]+ ... "
      ...
      Result := tregChars//No sigue ningún cuantificador, podrías er algún literal
      //Lista de tipo "[ ... ] ... "
    ;
  end
  else if (length(exp) = 1) and (exp[1] in ['*', '?', '+', '[']) then
  begin
...
{        if length(str) = 1 then begin  //verifica si tiene un caracter
      ...
      else if (tmp = '*') or (tmp = '?') or (tmp = '+') then
      begin
        ...
        begin
          ...
          begin  //verifica si tiene un caracter
            listChars := str;       //'['+str+']'
            Result := tregChars;   //devuelve como lista de un caracter
            exit;
          end;
        ...
        begin
          ...
          if tmp = '*' then Result := tregChars0_//Lista de tipo "[a]* ... "
          else if tmp = '?' then Result := tregChars01//Lista de tipo "[a]? ... "
          else if tmp = '+' then Result := tregChars1_//Lista de tipo "[a]+ ... "
          ;   //no hay otra opción
          exit;
        end;
      end;
...
function ColorFromStr(cad: string): TColor;
  ...
  begin
    ...
    for i := 1 to length(txt) do if not (txt[i] in ['0'..'9', 'a'..'f', 'A'..'F']) then exit(False)//no era
    ;
...
var
  r, g, b: integer;
begin
  if (cad <> '') and (cad[1] = '#') and (length(cad) = 7) then
  begin
...
var
  ...
  begin
    ...
    txt := trim(txt);
    if txt = '' then exit;
    if txt[1] = '(' then Result := True//caso esperado
    ;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if inst = 'NEXT' then Instrucs[n].actionMatch := aomNext//se pide avanzar al siguiente
      else if inst = 'EXIT' then
      begin  //se pide salir
        if HavePar(ifMatch) then
        begin  //EXIT con parámetro
          Instrucs[n].actionMatch := aomExitpar;
          Instrucs[n].destOnMatch := n + extractPar(ifMatch, ERR_SYN_PAR_IFMATCH_);
        end
        else
        begin   //EXIT sin parámetros
          Instrucs[n].actionMatch := aomExit;
        end;
      end
      else if inst = 'MOVE' then
      begin
  ...
  begin
    ...
    begin
      ...
      if inst = 'NEXT' then Instrucs[n].actionFail := aomNext//se pide avanzar al siguiente
      else if inst = 'EXIT' then
      begin  //se pide salir
        if HavePar(ifFail) then
        begin  //EXIT con parámetro
  ...
  case t of
    ...
    tregString: begin      //Es de tipo texto literal
      ...
      if CaseSensitive then Instrucs[n].Text := str
        ...
        Instrucs[n].Text := UpCase(str);  //ignora caja
    end;
    tregTokPos: begin
      n := AddItem(t, ifTrue, ifFalse) - 1;  //agrega
      Instrucs[n].aMatch := atMatch;
...
begin
  ...
  //expresión continua.
  if exp <> '' then raise ESynFacilSyn.Create(ERR_UNSUPPOR_EXP_ + expr);
end;
...
begin
  ...
  begin
    ...
    while exp <> '' do AddOneInstruct(exp, '', '')//en principio, siempre debe coger una expresión
...
begin
  Result := False;
  if TokPos = 0 then
  begin //búsqueda normal
    for i := 0 to High(mat) do if mat[i].txt = cad then
      begin
        n := i;
        exit(True);
      end;
    ...
    for i := 0 to High(mat) do if (mat[i].txt = cad) and (TokPos = mat[i].TokPos) then
      begin
        n := i;
        exit(True);
      end//búsqueda con TokPos
  ;
end;
...
begin
  //validaciones
  if tStart.hay and tCharsStart.hay then raise ESynFacilSyn.Create(ERR_NOT_USE_START)//No es un caso válido que se den los dos parámetros
  ;
  if not tStart.hay and not tCharsStart.hay then raise ESynFacilSyn.Create(ERR_PAR_START_CHARS)//Tampoco es un caso válido que no se de ninguno.
  ;
  //Hay uno u otro parámetro definido
  if tStart.hay then Result := EscapeText(tStart.val)//protege a los caracteres especiales
  else if tCharsStart.hay then Result := ToListRegex(tCharsStart)//convierte a expresión regular como [a..z]
  ;
end;
...
begin
  ...
  if delim[1] in charsIniIden then
  begin
    ...
    for c in tmp do
      if not CharsIdentif[c] then raise ESynFacilSyn.Create(format(ERR_TOK_DEL_IDE_ERR, [delim]));
  end;
end;
...
begin
  ...
  if t = tregChars then for c in listChars do ListElem.Add(c)
  else if t = tregString then
  begin  //lista simple o literal cadena
    VerifDelim(str);   //valida reglas
    lisTmp.Add(str);
  end
  else //expresión de otro tipo
    raise ESynFacilSyn.Create(ERR_EXP_MUST_BE_BR);
end;
...
begin
  ...
  if (delAct in [tdUniLin, tdMulLin]) and
    (delNue in [tdUniLin, tdMulLin]) then raise ESynFacilSyn.Create(Format(ERR_IDEN_ALREA_DEL, [delim]));
end;
...
begin
  Temp := fToIdent;
  if Length(r.txt) = fStringLen then
  begin  //primera comparación
  ...
  else  //definitívamente es diferente
    Result := False;
end;
...
var
  ...
  begin
    Result := True;  //valor por defecto
    num := 0; //valor por defecto
    for i := 1 to length(txt) do if not (txt[i] in ['0'..'9']) then exit(False)//no era
    ;
    //todos los dígitos son numéricos
    num := StrToInt(txt);
  end;
...
begin
  ...
  begin
    ...
    if UpCase(ansistring(atri.NodeName)) = UpCase(nomb) then
    begin
...
begin
  ...
  begin
    ...
    //verifica existencia
    hay := False;
    for j := 0 to lisTmp.Count - 1 do
    begin
      tmp := trim(lisTmp[j]);
      if nombre = UpCase(tmp) then
      begin
        hay := True;
        break;
      end;
    ...
    if not hay then raise ESynFacilSyn.Create(format(ERR_INVAL_ATTR_LAB, [atri.NodeName, n.NodeName]))//Este atributo está demás
    ;
  end;
end;
  ...
  begin
    ...
    case inst^.expTyp of
      ...
      tregString: begin  //texo literal
        ...
        if CaseSensitive then while (i < tam1) and (inst^.Text[i] = fLine[posFin]) do
          begin
            Inc(posFin);
            Inc(i);
          end//sensible a caja
        else
          while (i < tam1) and (inst^.Text[i] = TabMayusc[fLine[posFin]]) do
          begin
            Inc(posFin);
            Inc(i);
          end//Ignora mayúcula/minúscula
        ;
        //verifica la coincidencia
        if i = tam1 then
        begin //cumple
      ...
      tregChars: if inst^.Chars[fLine[posFin]] then
        begin
          //cumple el caracter
          if inst^.aMatch <> -1 then fTokenID := inst^.aMatch;  //pone atributo
        ...
        end;//conjunto de caracteres: [ ... ]
        //debe existir solo una vez

      tregChars01: begin   //conjunto de caracteres: [ ... ]?
        //debe existir cero o una vez
        if inst^.Chars[fLine[posFin]] then Inc(posFin)//pasa a la siguiente instrucción
        ;
      ...
      tregChars0_: begin   //conjunto de caracteres: [ ... ]*
        ...
        while inst^.Chars[fLine[posFin]] do Inc(posFin);
        //siempre cumplirá este tipo, no hay nada que verificar
        if inst^.aMatch <> -1 then fTokenID := inst^.aMatch;  //pone atributo
      ...
      tregChars1_: begin   //conjunto de caracteres: [ ... ]+
        ...
        while inst^.Chars[fLine[posFin]] do Inc(posFin);
        if posFin > posFin0 then
        begin   //Cumple el caracter
          if inst^.aMatch <> -1 then fTokenID := inst^.aMatch;  //pone atributo
          case inst^.actionMatch of
            aomNext: ;   //no hace nada, pasa al siguiente elemento
            aomExit: break;    //simplemente sale
...
begin
  ...
  charsIniIden := [];  //inicia
  for c in listChars do if c < #128 then
    begin  //caracter normal
      fProcTable[c] := @metIdent;
      charsIniIden += [c];  //agrega
    end
    else
    begin   //caracter UTF-8
      fProcTable[c] := @metIdentUTF8;
      charsIniIden += [c];  //agrega
    end//permite cualquier caracter inicial
  ;
  /////// Configura caracteres de contenido
  ...
  begin
    ...
    if CaseSensitive then
      ...
      TabMayusc[c] := UpCase(c)//pasamos todo a mayúscula
    ;
  end;
...
begin
  ...
  tkKeyword := NewTokAttrib('Reserved word', tnKeyword);      //atribuuto de palabras claves
...
begin
  ...
  for i := 0 to AttrCount - 1 do if Upcase(Attribute[i].Name) = txt then
    begin
...
begin
  ...
  for i := 0 to AttrCount - 1 do if Upcase(Attrib[i].Name) = txt then
    begin
...
begin
  ...
  ////////// cambia atributo //////////
  if IsAttributeName(tName.val) then tipTok := GetAttribByName(tName.val)//tipo de atributo
  else
    tipTok := NewTokAttrib(tName.val, tokId)//No existe, se crea.
  ;
  //obtiene referencia
  Atrib := tipTok;
  ...
  begin
    ...
    if tFrameEdg.hay then case UpCase(tFrameEdg.val) of
    ...
    if tFrameSty.hay then case UpCase(tFrameSty.val) of
    ...
    if tStyBold.hay then if tStyBold.bol then Atrib.Style := Atrib.Style + [fsBold]
      else
        Atrib.Style := Atrib.Style - [fsBold]//negrita
    ;
    if tStyItal.hay then if tStyItal.bol then Atrib.Style := Atrib.Style + [fsItalic]
      else
        Atrib.Style := Atrib.Style - [fsItalic]//cursiva
    ;
    if tStyUnder.hay then if tStyUnder.bol then Atrib.Style := Atrib.Style + [fsUnderline]
      else
        Atrib.Style := Atrib.Style - [fsUnderline]//subrayado
    ;
    if tStyStrike.hay then if tStyStrike.bol then Atrib.Style := Atrib.Style + [fsStrikeOut]
      else
        Atrib.Style := Atrib.Style - [fsStrikeOut]//tachado
    ;
    if tStyle.hay then
    begin  //forma alternativa
